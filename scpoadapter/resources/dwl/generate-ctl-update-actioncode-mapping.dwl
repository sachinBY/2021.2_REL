...
---
"options(skip = 1, errors=999999999," ++ "bindsize=" ++ p("sqlldr.bindsize") ++ ",readsize=" ++ p("sqlldr.readsize") ++", parallel=true) load data infile '" ++ (vars.updateCSVPath) ++ "'" ++ if (not (server.osName startsWith 'Windows')) "\"STR x'0D'\"" else "" ++ " append into table " ++ p('scpo.schema.staging.' ++ lower(vars.tableName) ++ '.update') ++ " fields terminated by ',' optionally enclosed by '\"' TRAILING NULLCOLS (" ++ columns ++ ")"
